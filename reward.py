...
def state_transition_reward(ref_code_tokens, pred_code_tokens) -> float:
    return execute_and_compute_state_score(convert_tokens_to_code(ref_code_tokens), convert_tokens_to_code(pred_code_tokens))
...
def blue_and_same_state_score(ref_code_tokens, pred_code_tokens, gamma) -> float:
    ...
    bleu = corpus_bleu([[ref_code_tokens]], [pred_code_tokens],
